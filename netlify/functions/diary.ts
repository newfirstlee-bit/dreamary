...
import { saveDiary, Diary, getDiaryByRequestId } from '../../src/lib/db';
...
export default async function reqHandler(req: Request) {
  ...
  try {
    ...
    if (requestId) {
      ...
      if (existing) {
        return new Response(JSON.stringify({ reply: existing.charReply, savedId: existing.id }), { headers: corsHeaders });
      }
    }
...
`;
    ...
    return new Response(JSON.stringify({ reply: charReply, savedId: newDiary.id }), { headers: corsHeaders });
  } catch (error: any) {
