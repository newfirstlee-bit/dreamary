...
- 로컬 린트 통과(기존 경고 있음), YAML 구문 확인, 소스 338개에서 알려진 실제 서버 비밀값 검출 0건. 업로드 후보 변경 103개와 파일 해시는 `artifacts/ci-2026-09-13/preflight.json`에 기록했다. Git 커밋·push·PR 생성·원격 CI 실행은 아직 하지 않았다.
...
1. 원격 GitHub CI 검증. `.github/workflows/verify.yml`은 아직 로컬 변경이므로 원격 실행 완료로 보고하지 않는다. 게시할 코드 변경 범위 확인과 저장소 인증이 선행 조건이다. 기존 저장소의 Netlify 자동 배포는 운영 사이트와 연결되어 있으므로 테스트 검증을 위해 무작정 push하지 않고 해당 배포 경로부터 분리/확인해야 한다.
