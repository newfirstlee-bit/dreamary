...
import { formatKoreanNameTemplate } from '../koreanJosa';
...
export async function buildDiaryPushCandidates(firestore: any, uid: string, locale = 'ko', todayDateString = getTodayKstDateString(), onlyCharacterId?: string) {
  const [charactersSnap, topicsSnap] = await Promise.all([
    onlyCharacterId
      ? firestore.collection('characters').doc(onlyCharacterId).get().then((snapshot: any) => ({
