...
import { corsHeaders } from '../../../netlify/shared/cors';
import { enqueueDataJob, advanceDataJob } from './dataJobs';
...
export default async function reqHandler(req: Request) {
  ...
  try {
    ...
    // Once accepted, server-owned state prevents new writes and survives logout.
    const job = await enqueueDataJob(firestore, 'account-delete', uid, '', '', async (tx, exists) => {
      if (!exists) tx.set(firestore.collection('accountStates').doc(uid), { deleting: true, requestedAt: Date.now() });
    });
    const { getAuth } = await import('firebase-admin/auth');
    await getAuth().updateUser(uid, { disabled: true });
    let done = false;
    const deadline = Date.now() + 8000;
    for (let step = 0; step < 25 && Date.now() < deadline; step++) {
      done = await advanceDataJob(firestore, job);
      if (done) break;
    }
    return Response.json({ success: true, accepted: true, done }, { headers: corsHeaders });
  } catch (error: any) {
