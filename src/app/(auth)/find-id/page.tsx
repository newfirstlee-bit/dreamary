  ...
  return (
    <div className="app-container" style={{ padding: '20px', display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: 'var(--bg-color)' }}>
      <header style={{ marginBottom: '40px', marginTop: '20px', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: 0 }}>{t('auth.findId')}</h1>
      </header>
        ...
        <form onSubmit={handleFindId} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <button
            ...
            style={{ 
              ...
              backgroundColor: loading || !email ? 'var(--gray-300)' : 'var(--point-color)', 
              color: 'white', fontSize: '1.1rem', fontWeight: 'bold', cursor: loading || !email ? 'not-allowed' : 'pointer'
            }}
          >
            {loading ? t('auth.sending') : t('auth.findId')}
          </button>
        </form>
      ) : (
        <div style={{ textAlign: 'center', marginTop: '40px' }}>
          <div style={{ fontSize: '3rem', marginBottom: '20px' }}>✉️</div>
          <h2 style={{ fontSize: '1.3rem', fontWeight: 'bold', marginBottom: '10px' }}>{t('auth.emailSent')}</h2>
          <p style={{ color: 'var(--gray-600)', marginBottom: '30px', lineHeight: '1.5' }} dangerouslySetInnerHTML={{ __html: t('auth.emailSentDesc') }} />
          <button
