...
- [x] 운영 release 빌드 `1.0.0` build 6 생성. iOS 최소 버전 15.0 Archive와 Android 서명 AAB를 검증했다. iOS는 App Store Connect 업로드 성공 후 처리 중이다.
- [ ] TestFlight 내부 설치 확인. 연결된 iPhone의 통신이 끊겨 로컬 덮어쓰기 설치는 보류했다.
- [ ] Google Play 내부 테스트 업로드. 서명 AAB는 준비됐고, 최초 업로드 전에 저장소 밖 업로드 키를 별도 보관해야 한다.
